...
class LSTM(nn.Module):
    """Standard LSTM model configuration to process sequential data."""
...
def load_embeddings(path):
    """
    Load and return the GloVe embeddings. Creates a .pkl for quicker re-accessing.
    """
    pkl = path[:-3] + "pkl"
...
def load_product_data(path):
    """
    Load and return the product data files.
    """
    print(f"[*] Loading {path}")
...
def sentence_embedding(sentence, embeddings, embedding_dim, max_len):
    """
    Create and return stacked and padded embedding for neural network input.

    sentence: The product description (str)
    embeddings: The GloVe word embeddings (dict)
    embedding_dim: The word embedding dimension (100) (int)
    max_len: The maximum embedding length size that should be considered (int)
    """
    if sentence is None:
...
def test_model(model, dataloader):
    """
    Function loop to validate and test the model.
    """
    model.eval()
    total_loss = 0
    correct_preds = 0
...
if __name__ == "__main__":
    ...
    for epoch in range(epochs):
        ...
        train_accuracy = correct_preds / (len(train_loader) * batch_size * num_classes)
