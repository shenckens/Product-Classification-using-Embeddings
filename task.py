...
def sentence_embedding(sentence, embeddings):
    """
    Embed sentence using GloVe word embeddings. Fill in zero-vector if the
    word is unknown.
...
def category_embeddings(product_data, embeddings):
    """
    Creates an average embedding for each category based on the products
    ...
    for product in products_test:
        ...
        if np.any(product_embedding):
            ...
            for cat, emb in cat_emb.items():
                similarities[cat] = cosine_similarity([product_embedding], [emb])[0][0]
